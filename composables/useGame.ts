...
export const handPosition = (pl: Hand): { left: string; top: string; transform: string } => {
...
const tableCardsAnimation = (table: Game["table"]) => {
...
const callBluffAnimation = (cb: () => void) => {
    ...
    tableCardsEl.animate(
        [
            ...
            {
                transform: "translateX(12px)  scale(1.06)",
                filter: "drop-shadow(0 0 22px  rgba(220,38,38,1))",
                offset: 0.11,
            },
            {
                transform: "translateX(-12px) scale(1.06)",
            ...
            },
            {
                ...
                filter: "drop-shadow(0 0 20px  rgba(220,38,38,0.95))",
                offset: 0.23,
            },
            {
            ...
            },
            {
                ...
                filter: "drop-shadow(0 0 18px  rgba(220,38,38,0.8))",
                offset: 0.52,
            },
            {
                transform: "translateX(6px)   scale(1.03)",
...
const dropCardsAnimation = (cb: () => void) => {
    const selectedCards =
        _game.hands.find((x) => x.player.id === _room.mainPlayer.id)?.cards?.filter((c) => c.selected) ?? [];
    ...
    elements.forEach((el, i) => {
        ...
        const targetX = tableRect
            ...
            : 0;
        const targetY = tableRect ? tableRect.top + tableRect.height / 2 - (cardRect.top + cardRect.height / 2) : -200;
        ...
        el.animate(
            [
                ...
                {
                    transform: `translate(${peakX}px, ${peakY}px) rotate(${rot * 0.5}deg) scale(1.06)`,
                    opacity: 1,
                    offset: 0.42,
                },
                {
                    transform: `translate(${targetX}px, ${targetY}px) rotate(${rot}deg) scale(0.82)`,
                    opacity: 0,
                    offset: 1,
                },
            ],
...
if (import.meta.client) {
    socket.on("game-started", (game: any) => {
        ...
        setTimeout(() => {
            dealingActive.value = false;
            startCountdown();
        }, dealDuration);
    });

    socket.on("cards-dropped", (game: any) => {
    ...
    socket.on("turn-timeout", ({ game, result }: { game: any; result: BluffResult }) => {
        ...
        triggerLifeLoss(result.loser.id);
        const timeoutEntry = {
            type: "timeout" as GameLogEntry["type"],
            text: `${result.loser.username}`,
            sub: "ran out of time — loses a life",
        };
        addLog(timeoutEntry);
