<template>
    <div class="relative flex items-center justify-center" style="width: 96px; height: 96px">
        <button
            @click="previousImage"
            :class="[
                'absolute left-0 text-lg text-pub-gold hover:text-pub-gold-bright transition-colors z-10 rounded-full p-1',
        ...
        <button
            @click="nextImage"
            :class="[
                'absolute right-0 text-lg text-pub-gold hover:text-pub-gold-bright transition-colors z-10 rounded-full p-1',
                onlyView ? 'invisible' : 'visible',
            ]"
            style="background: rgba(10, 6, 3, 0.7); border: 1px solid rgba(184, 134, 11, 0.3)"
        >
