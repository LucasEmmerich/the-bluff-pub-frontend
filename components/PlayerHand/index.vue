<template>     <div class="absolute"
        :class="isMainPlayer ? 'flex flex-col items-center' : 'flex items-center gap-1'"
        :style="{ ...positionStyle, filter: isCurrentTurn ? 'drop-shadow(0 0 18px rgba(212,168,32,0.9)) drop-shadow(0 0 6px rgba(212,168,32,0.6))' : undefined }">
                ...
                <div class="flex items-center gap-2 w-full">
                    ...
                    <div v-if="showCircle" class="relative" :style="`width: ${avatarPx}px; height: ${avatarPx}px;`">
                        ...
                        <div class="w-full h-full rounded-full overflow-hidden shadow-2xl"
                            ...
                            <VideoTile v-if="activeStream && (!isMainPlayer || isCamEnabled)" :stream="activeStream" :mirror="isMainPlayer" />
                    ...
                    <div v-if="isMainPlayer" class="flex flex-col items-center justify-between" style="flex: 1; min-width: 0; gap: 6px;">
                        ...
                        <div class="font-pub text-xs tracking-widest px-2 py-0.5 rounded text-center w-full truncate"
                            :style="isCurrentTurn
                    ...
                    <div v-if="isMainPlayer" class="flex flex-col gap-1">
                        <button
                            ...
                            <span class="tracking-wider" style="font-size: 9px; color: #86efac; font-weight: 700;">PLAY</span>
                        </button>
                        <button
                            class="action-btn flex items-center gap-2 px-3.5 rounded-lg font-pub"
                            ...
                            <span style="font-size: 1.1rem;">🫵</span>
                            <span class="tracking-wider" style="font-size: 9px; color: #fca5a5; font-weight: 700;">LIAR!</span>
                        </button>
                        <button
                            class="action-btn flex items-center gap-2 px-3.5 rounded-lg font-pub"
                            ...
                            <span class="tracking-wider" style="font-size: 9px; color: rgba(255,160,160,0.85); font-weight: 700;">GIVE UP</span>
                        </button>
                    </div>
...
<div v-if="activeLifeEvent" class="pointer-events-none select-none"
            ...
            <div v-if="!isMainPlayer" class="font-pub text-xs tracking-widest px-2 py-0.5 rounded text-center"
                :style="isCurrentTurn
                    ? 'background: rgba(20,10,0,0.8); border: 1px solid rgba(184,134,11,0.6); color: #f0c040;'
                    : 'background: rgba(0,0,0,0.75); border: 1px solid rgba(184,134,11,0.2); color: #c4a882;'">
                {{ playerCards.player.username }}
            ...
            <Teleport to="body">
                <div v-if="confirmGiveUp && isMainPlayer"
                    class="fixed inset-0 flex items-center justify-center"
                    style="z-index: 200; background: rgba(0,0,0,0.6);">
                    <div class="flex flex-col items-center gap-4 px-8 py-6 rounded-2xl font-pub"
                        style="background: rgba(120,10,10,0.97); border: 1px solid rgba(220,50,50,0.4); box-shadow: 0 0 60px rgba(180,0,0,0.5);">
                        <span class="text-base tracking-widest" style="color: #f0c040;">Give up?</span>
                        <span class="text-xs text-center" style="color: rgba(196,168,130,0.6); max-width: 180px;">You'll be eliminated from this match but stay in the room.</span>
                        <div class="flex gap-3">
                            <button class="action-btn px-4 py-1.5 rounded-xl text-xs tracking-wider"
                                style="background: rgba(180,20,20,0.85); border: 1px solid rgba(255,100,100,0.35); color: #fca5a5;"
                                @click="[giveUp(), confirmGiveUp = false]">
                                🏳️ Yes, give up
                            </button>
                            <button class="action-btn px-4 py-1.5 rounded-xl text-xs tracking-wider"
                                style="background: rgba(0,0,0,0.6); border: 1px solid rgba(184,134,11,0.3); color: rgba(196,168,130,0.8);"
        ...
        <div v-if="!isMainPlayer" class="flex mx-5" style="min-width: 80px; min-height: 60px;" :style="flip ? 'order: 0; justify-content: flex-end;' : ''">
            <img v-for="(card, i) in playerCards.cards" :key="card.id"
...
import { CARD_IMAGES, selectCard, handPosition, handFlip, lifeEvents, gamePhase, dropCards, giveUp, _game } from "~/composables/useGame";
