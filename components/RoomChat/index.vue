<template>
    <div class="flex flex-col rounded-xl overflow-hidden"
        style="width: 260px; height: 300px; background: rgba(8,5,3,0.92); border: 1px solid rgba(255,255,255,0.18); backdrop-filter: blur(10px); box-shadow: 0 4px 32px rgba(0,0,0,0.7);">
        ...
        <div class="flex shrink-0" style="border-bottom: 1px solid rgba(255,255,255,0.08);">
            <button
                ...
                {{ tab.label }}
                <span v-if="tab.id === 'room' && unread > 0 && activeTab !== 'room'"
                    class="flex items-center justify-center rounded-full font-pub text-[8px] font-black"
                    style="background: rgba(248,113,113,0.85); color: #fff; min-width: 14px; height: 14px; padding: 0 3px;">
                    {{ unread }}
                </span>
            </button>
        </div>
        ...
        <div v-if="activeTab === 'room'" class="flex flex-col flex-1 min-h-0">
            ...
            <div ref="msgList" class="flex-1 overflow-y-auto chat-scroll flex flex-col gap-px px-2 py-2" style="min-height: 0;">
                <div v-if="roomMessages.length === 0"
                ...
                <div v-for="msg in roomMessages" :key="msg.id"
                    ...
                    <img :src="avatars[msg.avatar]" class="shrink-0 rounded-full object-cover"
                        style="width: 18px; height: 18px; margin-top: 1px;" />
                    <div class="flex flex-col min-w-0">
                        <span class="font-pub text-[8px] tracking-wide leading-tight truncate"
                            :style="{ color: msg.playerId === mainPlayerId ? 'rgba(184,134,11,0.8)' : 'rgba(255,248,238,0.45)' }">
                            {{ msg.username }}
                        </span>
                        <span class="font-pub text-[10px] leading-snug break-words" style="color: rgba(255,248,238,0.75); word-break: break-word;">
                            {{ msg.text }}
                        </span>
                    </div>
                </div>
            ...
            <div class="shrink-0 px-2 pb-2 pt-1" style="border-top: 1px solid rgba(255,255,255,0.06);">
                <div class="flex gap-1.5 items-center rounded-lg px-2 py-1.5"
                    ...
                    <button @click="send"
                        ...
                        style="width: 20px; height: 20px; background: rgba(184,134,11,0.2); border: 1px solid rgba(184,134,11,0.3);">
                        <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="rgba(184,134,11,0.9)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div v-else class="flex-1 flex items-center justify-center flex-col gap-2">
            ...
            <span class="font-pub text-[9px] tracking-[0.3em] uppercase" style="color: rgba(255,248,238,0.2);">Coming soon</span>
        </div>
...
watch(() => roomMessages.length, async () => {
...
<style scoped>
input::placeholder { color: rgba(255,248,238,0.2); }
